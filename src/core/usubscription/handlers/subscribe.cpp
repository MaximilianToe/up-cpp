...
public:
	explicit ServiceInvocationError(const std::string& message)
...
// From helpers.rs
// template <typename T>
std::pair<uprotocol::datamodel::builder::Payload, uprotocol::v1::UUri>
extract_inputs(uint16_t expected_resource_id, uint16_t resource_id,
               const uprotocol::v1::UMessage& message) {
	auto payload = message.has_payload() ? std::make_optional(message.payload())
	                                     : std::nullopt;
	auto message_attributes =
	    message.has_attributes()
	        ? std::optional<uprotocol::v1::UAttributes>(message.attributes())
	        : std::nullopt;

	if (resource_id != expected_resource_id) {
		throw ServiceInvocationError::InvalidArgument(
		    "Wrong resource ID (expected " +
		    std::to_string(expected_resource_id) + ", got " +
	...
	try {
		...
		throw ServiceInvocationError::InvalidArgument(
		    "Expected NotificationsRequest payload, error when unpacking: " +
		    std::string(e.what()));
	}
	...
	// return {uprotocol::datamodel::builder::Payload(request),
...
std::optional<datamodel::builder::Payload> subscribe(
	...
	try {
		// Send subscription with se from v3::SubscriptionEvent with
		// subscription_sender from struct SubscriptionRequestHandler
	} catch (const std::exception& e) {
		std::cerr << "Error communicating with subscription manager: "
		          << e.what() << std::endl;
		return std::optional<datamodel::builder::Payload>(
		    ServiceInvocationError(
		        "Error processing request"));  // UStatus zurückgeben
	}
	try {
		...
		return std::optional<datamodel::builder::Payload>(
		    ServiceInvocationError("Error processing request"));
	}

	// Notify update channel
	// -> NotificationEvent StateChange/ Use notification_sender from struct
	...
	try {
		...
		std::cerr << "Error building response payload: " << e.what()
		          << std::endl;
		return std::optional<datamodel::builder::Payload>(
		    ServiceInvocationError("Error building response payload"));
	}
